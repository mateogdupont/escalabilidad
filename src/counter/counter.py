...
class Counter:
    ...
    def count_top(self, data_fragment: DataFragment, query_id):
        ...
        if not data_fragment.is_last(): 
            ...
            if added:
                ...
                to_log = f"TOP {top_amount} {df_str}"
                self.log_writer.log_counted_data(data_fragment, to_log)
        else:
    ...
    def count_type_3(self, data_fragment, query_id, queries, group_data, value, percentile, event):
        ...
        if not data_fragment.is_last():    
            ...
            to_log_3 = f"PERCENTILE {percentile} {value} {group_data}"
            self.log_writer.log_counted_data(data_fragment, to_log_3)
        else:
            sentiment_scores = {}
    ...
    def count_type_2(self, data_fragment, query_id, queries, group_data, value, event):
        ...
        if not data_fragment.is_last():
            ...
            to_log_2 = f"2 {value} {group_data}"
            self.log_writer.log_counted_data(data_fragment, to_log_2)
        else:
            next_id = data_fragment.get_id() + 1
            for group_data in self.counted_data[client_id][query_id].keys():
                if event.is_set():
        ...
        if not data_fragment.is_last():
            ...
            if type(group_data) == list:
                for data in group_data:
                    ...
                    to_log_1 = f"1 {value} {data}"
                    self.log_writer.log_counted_data(data_fragment, to_log_1)
            else:
